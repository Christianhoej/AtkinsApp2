package com.atkinsapp2.SwipBoxBridge;
...
import com.atkinsapp2.communications.CompartmentInterface;
import com.atkinsapp2.communications.CompartmentPresenter;
...
public class LockerManager extends ReactContextBaseJavaModule implements LockerCallback, CompartmentInterface {
    ...
    public void getData(String uid) {
        compartmentPresenter.getCompartments(uid);
    }
    ...
    public void onDataAvailable(DataModelCompartments dataModelCompartments) {
        if (dataModelCompartments != null && dataModelCompartments.getData().getCompartments().size() != 0) {
            WritableMap params = Arguments.createMap();
            params.putString(PARAM_TOKEN, dataModelCompartments.getData().getCompartments().get(0).getTokens().get(0).getToken());
            params.putString(PARAM_AUTHENTICATION_TOKEN, dataModelCompartments.getData().getCompartments().get(0).getTokens().get(0).getAuth().get(0).getChallenge());
            params.putString(PARAM_AUTHENTICATION_RESPONSE, dataModelCompartments.getData().getCompartments().get(0).getTokens().get(0).getAuth().get(0).getResponse());
